...
| 13 | Indexation — `www` → apex 301 redirect (GSC: "Alternate page with proper canonical tag" flagged across homepage/guides/roads/parks/privacy) | BLOCKED — `public_html/.htaccess` written + committed (`433e7d8e`, pushed) exactly per the approved plan, but `deploy.yml`'s FTP sync explicitly excludes `.htaccess` ("Never let the sync delete server-managed files" — line 43 of the exclude list), so the file never reached Hostinger. Confirmed live: `https://www.parkstatus.today/` still `200` after a successful deploy run. `deploy.yml` is on the protected list — needs the user's explicit call before any change there. | 2026-09-17 | — |
