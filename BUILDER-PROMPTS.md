...
| 13 | Indexation — `www` → apex 301 redirect (GSC: "Alternate page with proper canonical tag" flagged across homepage/guides/roads/parks/privacy) | DONE — `public_html/.htaccess` (new, one `mod_rewrite` rule): `www.parkstatus.today` (any scheme) → single-hop 301 to the same path on `https://parkstatus.today`. Apex `http`→`https` behavior unchanged. Live-verified post-deploy (curl). | 2026-09-17 | 2026-09-17 (pending commit) |
