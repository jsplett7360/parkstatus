...
| 8 | Seasonal guides (fee-free days, holiday hours, most-visited, open-in-winter) | DONE — 4 flat guides in `public_html/guides/` (fee-free-days-2026, open-holidays, most-visited [top-15 2025 NPS data], open-in-winter); on the existing guide template + BreadcrumbList/FAQPage JSON-LD; added to `guides/index.html`, `sitemap()`+`sitemap.xml`, `llmsTxt()`+`llms.txt`; every internal park/road link verified. Facts user-confirmed (2026 fee-free dates from nps.gov, 2025 visitation from NPS release + publiclandsdata). | 2026-09-08 | 2026-09-10 `f0b77797` (pushed) |
| 10 | Indexation — per-park distinctiveness pass (de-dupe + distinct line + facts row + title shortening) | DONE — `park-facts.json` (63 NPs) + `PARK_FACTS`; `.distinct` line (curated / first-sentence fallback) + `.pfacts` <dl> + JSON-LD `foundingDate`; de-duped "How we read" (names park + per-park tail) & "Before you go" (park-dependent cards) — harness 8/8 distinct; titles `Is <name> open? · Park Status Today` (2,066→~382 park pages >60ch). `parks*.json` byte-identical. | 2026-09-10 | 2026-09-10 `080ce3c8` (committed, not pushed) |
| 11 | Indexation — internal linking + /state/ hubs | DONE — `byState` map + `haversineMi`; `pageHtml` "Parks near here" (nearest 6 <250mi) + "More in <state>" + state-hub link; `stateHubHtml()`→`/state/<slug>/` (grouped, baked pills + blob refetch, BreadcrumbList+ItemList) + `stateIndexHtml()`→`/state/`; "States" in `siteNav()` + index.html header + footers; `sitemap()` +`states` param, `llms.txt` section; roadPageHtml → state hubs. `parks*.json` byte-identical. **NEEDS** `public_html/state` in `refresh-park-data.yml` git add. | 2026-09-10 | 2026-09-10 `8ca76182` (committed, not pushed) |
